...
class GPT_Model:
    ...
    def get_response(self, sample):
        ...
        text_instruct = messages[0]['content'][-1]['text']

        while attempt < self.retry_attempts:
            try:
                llm_inspect_prompt = text_instruct + 'According to the question, provide some information for vlm to better answer.'
                user_messages = [llm_inspect_prompt]
                conversation = [{"role": "system", "content": "You are a helpful assistant."}]
                for user_message in user_messages:
                    assistant_reply, conversation = create_chat_round(user_message, conversation, self.llm_client)
                messages[0]['content'][-1]['text'] += " Notation: " + assistant_reply
                ...
                # llm_inspect_prompt = f"{vlm_response}. '\n' This is the response provided by the multimodal large model: {self.model} for the input image when performing the task {text_instruct}. '\n' Please help identify any aspects that might be inaccurate or incomplete, and generate a prompt for it to re-run the task while double-checking its response."
                # user_messages = [llm_inspect_prompt]
                # conversation = [{"role": "system", "content": "You are a helpful assistant."}]
                # for user_message in user_messages:
                #     assistant_reply, conversation = create_chat_round(user_message, conversation, self.llm_client)
                    
                # messages[0]['content'][-1]['text'] = text_instruct + '\n' + " Notation: " + assistant_reply
                # response = self.client.chat.completions.create(
                #         model=self.model,
                #         messages=messages,
                #         temperature=self.temperature,
