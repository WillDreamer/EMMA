    ...
    def get_response(self, sample):
        ...
        while attempt < self.retry_attempts:
            try:
                ...
                llm_inspect_prompt = f"{vlm_response}. '\n' This is the response provided by the multimodal large model: {self.model} for the input image when performing the task {text_instruct}. '\n' The answerght be accuracte or inaccurate, and generate a prompt for it to rethink and re-run the task while double-checking its response."
                    
                messages[0]['content'][-1]['text'] = llm_inspect_prompt
                response = self.client.chat.completions.create(
                            model=self.model,
                            messages=messages,
                            temperature=self.temperature,
