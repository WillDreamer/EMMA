    ...
    def get_response(self, sample):
        ...
        while attempt < self.retry_attempts:
            try:
                ...
                llm_reason_supp = "{} '\n' According to the question, generate the chain of thoughts to solve this kind of problem".format(message_question)
                ...
                llm_receive_info = 'Large multi-modal model tells me: {} \n Large language model tells me: {} \n'.format(vlm_response, assistant_reply)

                # ask the question prompt to rethink
                messages[0]['content'][0]['text'] = llm_receive_info + "Re-think the following question: \n" + messages[0]['content'][0]['text']
                response_rethink = self.client.chat.completions.create(
                    model=self.model,
                    messages=messages,
                    temperature=self.temperature,
                    max_tokens=self.max_tokens,)
                vlm_response_rethink = response_rethink.choices[0].message.content.strip()

                return vlm_response_rethink, vlm_response, assistant_reply
